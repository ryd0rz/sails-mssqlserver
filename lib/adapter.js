...
module.exports = (function () {
  ...
  var adapter = {
     ...
     *
     */
    find: function (connection, collection, cb, options) {
      var collectionName = collection.using;
      options = options || _.extend({}, collection.criteria)
      ...
      var schemaName = getSchemaName(connection, collection)
      var tableName = '[' + schemaName + ']' + '.[' + collectionName + ']'
    ...
    getPrimaryKey: function (connection, collection) {
      var pk = 'id'
      var collectionName = typeof collection === 'object' ? collection.using : collection;
      Object.keys(connections[connection].collections[collectionName].definition).forEach(function (key) {
        if (!connections[connection].collections[collectionName].definition[key].hasOwnProperty('primaryKey')) return
        pk = key
      })
      return pk
    },
  ...
  function getSchemaName (connection, collection) {
    var collectionName = typeof collection === 'object' ? collection.using : collection;
    var collectionObject = connections[connection].collections[collectionName]
