      ...
      adapter.connectConnection(connection, function __FIND__ (err, uniqId) {
        ...
        if (data && data.length > 0) {
          for (var d = 0; d < data.length; d++) {
            if (data[d].key && data[d].value && data[d].type) {
              if (!sql[data[d].type]) {
                console.error('Data type specified does not exist: ', data[d].type);
              }
              request.input(data[d].key, sql[data[d].type], data[d].value);
            } else if (data[d].key && data[d].value) {
              request.input(data[d].key, data[d].value);
            } else {
              // insufficent information to use data
            }
          }
        }
